...
let turn: picture => picture = p => Box.turn >> p;

let flip: picture => picture = p => Box.flip >> p;

let toss: picture => picture = p => Box.toss >> p;
...
let above: (picture, picture) => picture = aboveRatio(1, 1);
...
let beside: (picture, picture) => picture = besideRatio(1, 1);

let quartet: (picture, picture, picture, picture) => picture =
  (nw, ne, sw, se) => above(beside(nw, ne), beside(sw, se));
...
let over: (picture, picture) => picture =
  (p1, p2, box) => List.append(p1(box), p2(box));
