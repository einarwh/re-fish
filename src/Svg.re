...
let toElement: StyledShape.styledShape => element =
  styledShape => {
    ...
    switch (shape) {
    | Shape.LineShape(_line) =>
      LineElement({x1: "0", y1: "0", x2: "0", y2: "0", stroke: "pink"})
    | Shape.PolygonShape(polygon) =>
      let points = Polygon.points(polygon);
      let pointsStr = toPointsString(points);
      PolygonElement({points: pointsStr, stroke: strokeColor, strokeWidth});
    | Shape.PolylineShape(polyline) =>
      let points = Polyline.points(polyline);
      let pointsStr = toPointsString(points);
      PolylineElement({points: pointsStr, stroke: strokeColor, strokeWidth});
    | Shape.CurveShape(curve) =>
      ...
      PathElement({d, stroke: strokeColor, strokeWidth});
    | Shape.PathShape(_start, _beziers) =>
      let d = "M 0,0";
      PathElement({d, stroke: strokeColor, strokeWidth});
    };
  };
...
let lines: list(element) => list(lineAttributes) =
  elements => choose(selectLineAttributes, elements);

let polygons: list(element) => list(polygonAttributes) =
  elements => choose(selectPolygonAttributes, elements);

let polylines: list(element) => list(polylineAttributes) =
  elements => choose(selectPolylineAttributes, elements);

let paths: list(element) => list(pathAttributes) =
  elements => choose(selectPathAttributes, elements);