...
let create: (vector, vector, vector) => box = (a, b, c) => {a, b, c};

let turn: box => box = ({a, b, c}) => {a: add(a, b), b: c, c: neg(b)};

let flip: box => box = ({a, b, c}) => {a: add(a, b), b: neg(b), c};
...
let moveVertically: (float, box) => box =
  (f, {a, b, c}) => {a: add(a, scale(f, c)), b, c};

let scaleVertically: (float, box) => box =
  (f, {a, b, c}) => {a, b, c: scale(f, c)};
...
let moveHorizontally: (float, box) => box =
  (f, {a, b, c}) => {a: add(a, scale(f, b)), b, c};

let scaleHorizontally: (float, box) => box =
  (f, {a, b, c}) => {a, b: scale(f, b), c};
...
let a: box => vector = box => box.a;

let b: box => vector = box => box.b;

let c: box => vector = box => box.c;